...
namespace AquariumMonitor.DAL
{
    public class WaterChangeRepository : BaseRepository, IWaterChangeRepository
    {
        private const string GetByAllForAquariumQuery = @"SELECT wc.Id,AquariumId,PercentageChanged,Changed,a.UserId,wc.[RowVersion]
                                                        FROM dbo.WaterChanges wc
                                                        JOIN dbo.Aquariums a ON a.Id = wc.AquariumId
        ...
        private const string InsertQuery = @"INSERT INTO dbo.WaterChanges (UserId,AquariumId,PercentageChanged,Changed)
                                             VALUES (@UserId,@AquariumId,@PercentageChanged,@Changed);
                                             SELECT Id, RowVersion FROM dbo.WaterChanges WHERE Id = CAST(SCOPE_IDENTITY() AS INT);";

        private const string UpdateQuery = @"UPDATE dbo.WaterChanges SET PercentageChanged = @PercentageChanged,Changed = @Changed
                                             WHERE Id = @Id;
                                             SELECT RowVersion FROM dbo.WaterChanges WHERE Id = @Id";

        private const string DeleteQuery = @"UPDATE dbo.WaterChanges SET Deleted = 1 WHERE Id = @Id;";
